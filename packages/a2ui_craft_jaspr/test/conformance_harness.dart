...
/// The Jaspr conformance harness: a [CraftTester] over Jaspr's own
/// [ComponentTester], and the [CraftConformanceDriver] that registers cases
/// with Jaspr's test runner.
library;
...
/// Builds the Jaspr conformance driver.
///
/// Lives here, apart from any one suite, because more than one test file runs
/// the shared suites through it — the same harness, a different runner.
CraftConformanceDriver jasprConformanceDriver() => JasprConformanceDriver();
...
class JasprCraftTester implements CraftTester {
  JasprCraftTester(this._tester);
...
/// Owns the mounted surface's theme and media so [JasprCraftTester.retheme] /
/// [JasprCraftTester.remedia] can swap them in place (setState) — pumping a
...
class JasprConformanceDriver implements CraftConformanceDriver {
  @override
  void defineTest(
    String description,
    ...
    testComponents(
      description,
      (ComponentTester tester) => body(JasprCraftTester(tester)),
    );
  }
}
