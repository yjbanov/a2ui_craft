...
6. **Parity test** in `test/`: render the *same* template used by the Flutter
   adapter's `test/remote_component_test.dart` (Column + data-bound Text + Button
   that dispatches an event) and assert: initial render, event reaches `onEvent`,
   and a `DynamicContent` update re-renders the bound text. It must pass before
   the adapter is considered working.

## Verify

- The new adapter's test passes.
- `dart test packages/a2ui_craft` and the existing adapters still pass.
- `flutter pub get` (or the appropriate resolver) solves the whole workspace.
- The same counter/greeting template renders equivalently on the new adapter and
  the existing ones.
