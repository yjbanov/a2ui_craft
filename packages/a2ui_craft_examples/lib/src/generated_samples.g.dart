...
// Calculator and the key itself is stateless. The keypad token colors the
// Button's own surface (the control owns background, corner shape, and
// press feedback; the child is content only) — a fixed-size Box keeps every
// key the same size, so the Button's stock padding is switched off.
widget Key = Button(
...
// A motion demo. Two ways `Box(animate:)` reads as motion:
//   1. Movement — press "Animate" and each bar races the *same* width change
//      (12 → 320) through a different easing curve, so you can compare them.
//   2. Cross-fade — flip the light/dark mode and the themed card eases its
//      surface + border over 1.5s instead of snapping.
// `{ go: false }` is local state; the button flips it with `set state.go`. Each
// bar sits in its own Column (a vertical flex) so its explicit width is a
// cross-axis size that holds and animates, rather than a shrinkable Row item.
widget Motion { go: false } =
  Column(gap: 24.0, crossAxisAlignment: "start", children: [
    Text(text: "Easing curves — press Animate to race the same width change through each curve:"),
    Button(
      onPressed: set state.go = switch state.go { true: false, default: true },
      child: Text(text: switch state.go { true: "Reset", default: "Animate" }),
    ),
    Column(gap: 14.0, crossAxisAlignment: "start", children: [
      Column(crossAxisAlignment: "start", gap: 4.0, children: [
        Text(text: "linear"),
        Box(
          width: switch state.go { true: 320.0, default: 12.0 },
          height: 22.0, cornerRadius: 6.0, color: "#EF4444",
          animate: { duration: 1200, easing: "linear" },
        ),
      ]),
      Column(crossAxisAlignment: "start", gap: 4.0, children: [
        Text(text: "standard"),
        Box(
          width: switch state.go { true: 320.0, default: 12.0 },
          height: 22.0, cornerRadius: 6.0, color: "#3B82F6",
          animate: { duration: 1200, easing: "standard" },
        ),
      ]),
      Column(crossAxisAlignment: "start", gap: 4.0, children: [
        ...
        Box(
          width: switch state.go { true: 320.0, default: 12.0 },
          height: 22.0, cornerRadius: 6.0, color: "#8B5CF6",
          animate: { duration: 1200, easing: "emphasized" },
        ),
      ]),
      Column(crossAxisAlignment: "start", gap: 4.0, children: [
        ...
        Box(
          width: switch state.go { true: 320.0, default: 12.0 },
          height: 22.0, cornerRadius: 6.0, color: "#10B981",
          animate: { duration: 1200, easing: "decelerate" },
        ),
      ]),
      Column(crossAxisAlignment: "start", gap: 4.0, children: [
        ...
        Box(
          width: switch state.go { true: 320.0, default: 12.0 },
          height: 22.0, cornerRadius: 6.0, color: "#F59E0B",
          animate: { duration: 1200, easing: "accelerate" },
        ),
      ]),
    ]),
    Divider(),
    Text(text: "Theme cross-fade — flip the light/dark mode; this card eases its surface and border over 1.5s:"),
    Box(
      width: 260.0,
      padding: [20.0, 20.0, 20.0, 20.0],
      cornerRadius: 16.0,
      ...
      border: 2,
      animate: { duration: 1500, easing: "standard" },
      child: Text(text: "themed surface"),
    ),
  ]);
''',
    schema: r'''
...
''',
    demonstrates: <String>['layout', 'controls', 'theming'],
  ),
  RawSample(
    id: 'layout',
    label: 'Layout',
...
// Two complementary responsive tools, both driven by the host's window size
// class (research/responsive/RESPONSIVE_DESIGN.md). Flip the size class in the
// toolbar to see both react in place, identically on Flutter and Jaspr:
//
//  1. The `Responsive` **primitive** swaps a whole subtree — the metric cards
//     **stack** in a Column when compact and sit **side by side** in a Row when
//     expanded, with no template branching.
//  2. The `media.` reference **scope** branches a single prop: the caption reads
//     the live `media.width` id, and the outer gap widens once `atLeast` reports
//     the width has reached `expanded`.
...
widget Dashboard = Column(
  gap: switch atLeast(a: media.width, b: "expanded") { true: 20.0, default: 12.0 },
  children: [
  Heading(text: "Store metrics", level: 3),
  Text(text: concat(a: "Size class: ", b: media.width), variant: "caption"),
  Responsive(
...
// Intrinsic responsiveness with **no breakpoints**: `Grid` lays out as many
// equal columns as fit at ≥ `minColumnWidth`, then reflows as the width changes
// — identically on Flutter and Jaspr (research/responsive/RESPONSIVE_DESIGN.md
// §4.1). Unlike the `responsive` sample (which restructures at host-supplied
// size classes), this needs no size input at all: narrow the pane and the same
// cards repack from four columns to one on their own.
...
widget Dashboard = Column(gap: 12.0, children: [
  Heading(text: "Team dashboard", level: 3),
  Grid(minColumnWidth: 160.0, gap: 12.0, runGap: 12.0, children: [
    Metric(label: "Revenue", value: "$48.2k"),
    Metric(label: "Orders", value: "1,284"),
    Metric(label: "Refunds", value: "12"),
    Metric(label: "Visitors", value: "92k"),
    Metric(label: "Signups", value: "438"),
    Metric(label: "Churn", value: "2.1%"),
  ]),
]);
''',
    schema: r'''
...
class RawMiniApp {
  ...
  /// The JavaScript driver, as source — the portable half of
  /// the mini-app, runnable in a worker on any host.
