// Copyright 2013 The Flutter Authors
// Use of this source code is governed by a BSD-style license that can be
// found in the LICENSE file.

/// Shared, framework-neutral A2UI Craft sample definitions.
///
/// Each [SampleSpec] is a self-contained demo as data — an RFW template, its
/// component API as JSON Schema, an A2UI message script, and an action handler.
/// The Flutter and Jaspr example galleries each render these through a thin
/// per-framework `Sample` widget, so the samples are defined exactly once.
library a2ui_craft_examples;

export 'src/sample_spec.dart';
export 'src/samples.dart';
export 'src/samples/account_balance.dart';
export 'src/samples/boxes.dart';
export 'src/samples/calendar_day.dart';
export 'src/samples/chat_message.dart';
export 'src/samples/child_list_template.dart';
export 'src/samples/coffee_order.dart';
export 'src/samples/complex_layout.dart';
export 'src/samples/contact_card.dart';
export 'src/samples/countdown_timer.dart';
export 'src/samples/counter.dart';
export 'src/samples/credit_card.dart';
export 'src/samples/email_compose.dart';
export 'src/samples/event_detail.dart';
export 'src/samples/financial_data_grid.dart';
export 'src/samples/flight_status.dart';
export 'src/samples/form.dart';
export 'src/samples/form_validator.dart';
export 'src/samples/formatted_text.dart';
export 'src/samples/gallery.dart';
export 'src/samples/greeting.dart';
export 'src/samples/incremental.dart';
export 'src/samples/incremental_dashboard.dart';
export 'src/samples/interactive_button.dart';
export 'src/samples/layout.dart';
export 'src/samples/login_form.dart';
export 'src/samples/markdown_text.dart';
export 'src/samples/music_player.dart';
export 'src/samples/notification_permission.dart';
export 'src/samples/product_card.dart';
export 'src/samples/profile_card.dart';
export 'src/samples/purchase_complete.dart';
export 'src/samples/restaurant_card.dart';
export 'src/samples/row_layout.dart';
export 'src/samples/shipping_status.dart';
export 'src/samples/sign_in.dart';
export 'src/samples/simple_text.dart';
export 'src/samples/sports_player.dart';
export 'src/samples/stats_card.dart';
export 'src/samples/step_counter.dart';
export 'src/samples/track_list.dart';
export 'src/samples/user_profile.dart';
export 'src/samples/weather.dart';
export 'src/samples/workout_summary.dart';
